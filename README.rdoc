...
The default "owner" of a locked semaphore is "Machine uuid" + "process pid" + "fiber id".
Redis::EM::Mutex allows you to replace the last part of the owner with your own value.
To customize mutex "owner" you should use :owner option of newly created mutex,
passing some well defined owner id.
In our case the "scope" of a mutex will be the client's ip address and the "owner id"
will be an __id__ of EventMachine::Connection object created upon connection from the client.
...
can access mutex's #refresh and #unlock methods concurrently.
  ...
  EventMachine.run {
    EventMachine.start_server "0.0.0.0", 6969, MutexedConnection
  }
